...
* **Hạn chế & Điểm nghẽn:**
  * *Mật độ chữ quá cao:* Du khách ở thực địa rất khó đọc hết các đoạn văn bản dài.
...
* **Cải tiến:** Visual-first, function calling + UI state (`tool-*` parts trong `UIMessage`).
...
* **Tài liệu kỹ thuật:** `reports/VINWONDERS_AGENT_INDIVIDUAL_REPORT.md`, `reports/GROUP_REPORT.md`.
...
## 6. Báo cáo nhóm (Reports)

| Báo cáo | Đường dẫn |
|---------|-----------|
| Individual report | `reports/VINWONDERS_AGENT_INDIVIDUAL_REPORT.md` |
| Group report | `reports/GROUP_REPORT.md` |

---

## 7. Xem codebase trên terminal (có màu)

Chạy từ thư mục gốc repo:

```bash
./scripts/codebase-map.sh
```

Tắt màu (CI / log file):

```bash
NO_COLOR=1 ./scripts/codebase-map.sh
```

**Thẻ màu:** `▌ API` (xanh dương) · `▌ GUARD` (đỏ) · `▌ TOOLS` (xanh lá) · `▌ UI` (tím) · `▌ MEMORY/LOG` (cyan) · `▌ DOCS` (trắng)
